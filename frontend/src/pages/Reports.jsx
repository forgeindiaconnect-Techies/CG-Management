...
export default function Reports() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Filter only complaints assigned to this user if Staff
        const myComplaints = user.role === 'Staff' 
  ...
  const handleExportPDF = async () => {
    try {
      ...
      alert('Export feature requires backend support (/api/exports/pdf)');
    } finally {
  ...
  const handleExportExcel = async () => {
    try {
      ...
      alert('Export feature requires backend support (/api/exports/excel)');
    } finally {
    ...
    <div className="space-y-6 max-w-6xl mx-auto">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">View your statistics and export analytics.</p>
        </div>
        <div className="flex items-center gap-3">
          <button
            onClick={handleExportPDF}
            disabled={exportingPDF}
            className="inline-flex items-center gap-1.5 px-4 py-2 border border-teal-200 text-teal-700 text-sm font-semibold rounded-xl bg-teal-50/30 hover:bg-teal-50 transition-colors disabled:opacity-50"
          >
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 px-4 py-2 border border-teal-200 text-teal-700 text-sm font-semibold rounded-xl bg-teal-50/30 hover:bg-teal-50 transition-colors disabled:opacity-50"
          >
            <FileSpreadsheet size={16} /> {exportingExcel ? 'Generating...' : 'Export Excel'}
          </button>
        </div>
      </div>
          ...
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={trendData}>
                ...
                <Tooltip />
                <Line type="monotone" dataKey="resolved" stroke="#0F766E" strokeWidth={3} dot={{ r: 4, fill: '#0F766E' }} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </div>
