...
import { Plus, Edit2, Power, Search, Shield, Building2, Trash2 } from 'lucide-react';
...
export default function DeptHeadManagement() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      setDeptHeads(staffRes.data.filter(u => u.role === 'Department Head'));
  ...
  const openAddModal = () => {
    setModalMode('add');
    setFormData({ id: null, name: '', email: '', password: '', role: 'Department Head', department_id: '' });
    setShowModal(true);
  };
  ...
  const filteredHeads = deptHeads.filter(s => 
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-black text-gray-900">Department Head Management</h2>
          <p className="text-gray-500 text-sm mt-0.5">Manage department heads and assign departments</p>
        </div>
        <button 
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm text-gray-600">
            ...
            <tbody className="divide-y divide-gray-100">
              {loading ? (
                <tr>
                  <td colSpan="5" className="px-6 py-10 text-center text-gray-400">Loading department heads...</td>
                </tr>
              ) : filteredHeads.length === 0 ? (
                <tr>
                  <td colSpan="5" className="px-6 py-10 text-center text-gray-400">No department heads found.</td>
                </tr>
              ) : filteredHeads.map(user => (
                <tr key={user._id} className="hover:bg-gray-50/50 transition-colors">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-full bg-gradient-to-br from-purple-500 to-indigo-500 flex items-center justify-center text-white font-bold text-xs shadow-sm">
                  ...
                  <td className="px-6 py-4 font-medium text-gray-700">
                    <div className="flex items-center gap-2">
                      <Building2 size={14} className={user.department_id ? "text-gray-400" : "text-red-400"} />
                      {user.department_id ? user.department_id.name : <span className="text-red-400 italic">Needs Assignment</span>}
                    </div>
                  </td>
                  <td className="px-6 py-4">
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            <div className="p-5 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
              <h3 className="font-bold text-gray-900 text-lg">
                {modalMode === 'add' ? 'Add Department Head' : 'Edit / Assign Department'}
              </h3>
              <button 
            ...
            <form onSubmit={handleSubmit} className="p-5 space-y-4">
              ...
              <div>
                <label className="block text-xs font-bold text-gray-700 uppercase tracking-wide mb-1.5">Assign Department</label>
                <select 
                ...
                <p className="text-xs text-gray-500 mt-1.5">
                  {modalMode === 'edit' && !formData.department_id ? "This user needs a department assignment to function as a Department Head." : ""}
                </p>
              </div>
